...
public class StochasticHillClimbingAgent extends Agent {
	...
	public StochasticHillClimbingAgent(int playernum) {
		...
		System.out.println("Constructed StochasticHillClimbingAgent");
	}
	...
	public Map<Integer, Action> middleStep(State.StateView newstate, History.HistoryView statehistory) {
		...
		if (this.getIsStuck()) {
			...
			actions.put(this.getFootmanID(), Action.createPrimitiveAttack(this.getFootmanID(), this.getFootmanID()));
		} else if (townhallUnit != null) // townhall is still alive
		{
			...
			if (Math.abs(footmanX - townhallUnit.getXPosition()) <= 1
				...
				actions.put(this.getFootmanID(),
						Action.createPrimitiveAttack(this.getFootmanID(), this.getEnemyTownhallID()));
			} else // not adjacent to townhall and townhall is still alive...move there
			{
				if (this.getCurrentDesiredPosition() != null) {
					...
					if (footmanX == this.getCurrentDesiredPosition().getXPosition()
						...
						// System.out.println("creating primitive action to move in direction " +
						// nextDirection);

						actions.put(this.getFootmanID(),
								Action.createPrimitiveMove(this.getFootmanID(), nextDirection));
					}
				}
				...
				if (this.getCurrentDesiredPosition() == null) {
					...
					this.setCurrentDesiredPosition(this.getNextPosition(new Coordinate(footmanX, footmanY),
							new Coordinate(townhallUnit.getXPosition(), townhallUnit.getYPosition()),
							newstate.getXExtent(), newstate.getYExtent(), this.getObstacles()));
					if (this.getCurrentDesiredPosition() != null) {
	...
	/**
	 * A method that converts a list of N utilities into a list of N probabilities.
	 * This method assumes that lower utilities are better, so it converts smaller
	 * utility values into larger probabilities. This is done with the following
	 * equation: prob[position i] = (1.0 / utility[position i]) / (sum from j=1 -> N
	 * of (1.0 / utility[position j]))
	 * 
	 * @param utilities. The list of utility values.
	 * @return List<Double> the list of probabilities.
	 */
	private List<Double> utilitiesToProbabilities(List<Float> utilities) {
		// want lower utility values to correspond to higher probabilities
		// first I'll invert the utility values and then map them to probabilities using
		// a l_1 norm
		double sum = 0.0;
	...
	/**
	 * A method for picking a child at random where the probability of a child is
	 * also specified. This is done by first argsorting the probabilities in
	 * descending order. We then conver the probabilities (which are a pmf) into a
	 * cmf in descending order. By choosing a random number, we can incorporate the
	 * "weight" of each child by finding the position j in the cmf where the random
	 * value is greater than the cmf value. We can then use that position j to look
	 * up the corresponding position in the argsort and finally the child that
	 * corresponds to that argsorted position.
	 * 
	 * @param children. The list of children to consider.
	 * @param probs.    The probability of each child. This should be a pmf.
	 * @return Coordinate. The chosen child.
	 */
	private Coordinate pickRandomChild(List<Coordinate> children, final List<Double> probs) {
		Coordinate child = null;
		if (children.size() == 1) {
			...
			// get random double and find the largest index j such that randValue <=
			// cdf[j+1]
			...
			if (j == cdf.size() - 1 || j == 0) {
				; // do nothing if we reached the end of the cdf...j is the proper position (edge
					// case)
			} else {
	...
	/**
	 * TODO: calculate the coordinate of the square to move to. Our code will
	 * automatically take control once you are adjacent to the townhall and will
	 * kill it.
	 * 
	 ...
	 * @param obstacles: set of obstacles. These obstacles prevent you from
	 *                   occupying squares in the map.
	 * @return
	 */
	private Coordinate getNextPosition(Coordinate pt, Coordinate goalPt, int xExtent, int yExtent,
	 ...
	 * @param xExtent   The maximum x boundary of the environment (assuming 0 is
	 *                  min).
	 * @param yExtent   The maximum y boundary of the environment (assuming 0 is
	 *                  min).
	 * @param obstacles A set of obstacles in the environment represented as a set
	 *                  of positions that are already occupied.
	 * @return ArrayList of valid positions representing unoccupied neighbors of
	 *         node pt.
	 */
	private ArrayList<Coordinate> getAndCheckNeighbors(Coordinate pt, Coordinate goal, int xExtent, int yExtent,
			Set<Coordinate> obstacles) {
		...
		for (int i = pt.getXPosition() - 1; i <= pt.getXPosition() + 1; i++) { // run from pt.x-1 -> pt.x+1
			for (int j = pt.getYPosition() - 1; j <= pt.getYPosition() + 1; j++) { // run from pt.y-1 -> pt.y+1
				if (i != pt.getXPosition() || j != pt.getYPosition()) { // if we aren't at (pt.x, pt.y)
					newPt = new Coordinate(i, j); // neighbor point

					// if the point is valid (not occupied and in range)
					if (newPt.getXPosition() >= 0 && newPt.getXPosition() < xExtent && newPt.getYPosition() >= 0
							&& newPt.getYPosition() < yExtent && !obstacles.contains(newPt)) {
