...
public class QAgent extends Agent {
	...
	private int hidden_dim1 = 5;
	private int hidden_dim2 = 4;
	private int hidden_dim3 = 2;

	private static final int FRIENDLY_UNIT_RADIUS = 10;
	...
	public QAgent(int playerId, String[] args) {
		...
		this.optimizer = new SGDOptimizer(this.getQFunction().getParameters(), QAgent.LEARNING_RATE, 10000); // altered
																												// and
																												// added
																												// 10000
	...
	private Model initializeQFunction(boolean loadParams) {
		 ...
		 */
		// layer 1
		m.add(new Dense(NUM_FEATURES, hidden_dim1, this.getRandom()));
		m.add(new Sigmoid());
	...
	private double getRewardForUnit(StateView state, HistoryView history, int unitId) {
		...
		int playerNum = -1;
		if (unit != null) {
			playerNum = unit.getTemplateView().getPlayer();
		}

		// Check if the unit took or gave damage in the last turn
		for (DamageLog damageLog : history.getDamageLogs(lastTurnNumber)) {
			if (damageLog.getDefenderID() == unitId) {
				if (playerNum == damageLog.getDefenderController()) {
					reward -= 5 * damageLog.getDamage(); // penalty for taking damage
				} else {
					reward += 50 * damageLog.getDamage(); // reward for dealing damage
				}
			} else if (damageLog.getAttackerID() == unitId) {
		...
		// Check if the unit died or killed its enemy in the last turn
		for (DeathLog deathLog : history.getDeathLogs(lastTurnNumber)) {
			if (deathLog.getDeadUnitID() == unitId) {
				if (playerNum == deathLog.getController()) {
					...
					reward += 1000.0; // reward for making a kill
				}
			} else if (state.getUnit(deathLog.getDeadUnitID()) != null
					&& state.getUnit(deathLog.getDeadUnitID()).getTemplateView().getPlayer() == playerNum) {
				if (playerNum == deathLog.getController()) {
					reward += 1000.0; // reward for making a kill
				} else {
					reward -= 100.0; // penalty for an ally dying
				}
			}
		}

		// reward for each action taken by the footman
		Map<Integer, Action> actionsTaken = history.getCommandsIssued(playerNum, lastTurnNumber);
		if (actionsTaken.containsKey(unitId)) {
			reward += 10.0;
		}

		// Discount the reward based on the timestep it is received on
		reward *= Math.pow(0.99, state.getTurnNumber());
	...
	private Matrix calculateFeatureVector(StateView state, HistoryView history, int atkUnitId, int tgtUnitId) {
		/** TODO: complete me! **/
		/**
		 * We chose these features to calculate and control the interactions between the
		 * target units. By knowing the distance and health of the units our agent
		 * should be able to capitalize and be rewarded for its actions
		 **/
		// initialize the feature vector with a constant 1
		// Initialize the feature vector with a constant 1
		Matrix featureVector = Matrix.zeros(1, NUM_FEATURES);
		...
		double distance = Math.sqrt(Math.pow(attackingUnit.getXPosition() - targetUnit.getXPosition(), 2)
				+ Math.pow(attackingUnit.getYPosition() - targetUnit.getYPosition(), 2));
		featureVector.set(0, 1, distance);

		// Get the health of the attacking and target units
		double attackingHealth = attackingUnit.getHP();
		double targetHealth = targetUnit.getHP();
		featureVector.set(0, 2, attackingHealth);
		featureVector.set(0, 3, targetHealth);

		// Calculate the number of friendly units within a certain radius of the
		// attacking unit
		int numFriendlyUnits = 0;
		for (UnitView unit : state.getAllUnits()) {
			if (unit.getTemplateView().getPlayer() == attackingUnit.getTemplateView().getPlayer()
					&& unit.getID() != atkUnitId) {
				double unitDistance = Math.sqrt(Math.pow(attackingUnit.getXPosition() - unit.getXPosition(), 2)
						+ Math.pow(attackingUnit.getYPosition() - unit.getYPosition(), 2));
				if (unitDistance <= FRIENDLY_UNIT_RADIUS) {
					numFriendlyUnits++;
				}
			}
		}
		featureVector.set(0, 4, numFriendlyUnits);
