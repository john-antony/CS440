...
public class AStarAgent extends Agent {
	...
	/**
	 * A class to represent the location of an object in a map. Stores x, y position
	 * as well as the MapLocation that came before it (for path searching), f(n) and
	 * g(n) of the node.
	 * 
	 * @author Andrew Wood
	 * @author Daniel Kim
	...
	class MapLocation {
		...
		public float g; // dijkstra's path cost: the cost of the path from the src up to the current
						// node
		public float f; // A* estimate vertex cost: the sum of g + h. H is only used here to help A*
						// make
		...
		public String toString() {
			return "(" + this.x + ", " + this.y + ", g=" + this.g + ", f=" + this.f + ")";
		}
		 ...
		 * in a HashMap or HashSet. The function is (x^2)*(y^3) because it is a good one
	...
	class costComparator implements Comparator<MapLocation> {
		public int compare(MapLocation o1, MapLocation o2) {
			// TODO Auto-generated method stub
			return Float.compare(o1.g, o2.g);
		}
	}
	 ...
	 * 
	 * Time Complexity: O(n) because the contains method has a time complexity of
	 * O(n).
	 *
	...
	private boolean shouldReplanPath(State.StateView state, History.HistoryView history,
		...
		// Initialize enemy footman and get location of enemy footman.
		Unit.UnitView enemyFootman = state.getUnit(this.enemyFootmanID);
		MapLocation enemyLoc = new MapLocation(enemyFootman.getXPosition(), enemyFootman.getYPosition(), null);
		// check if the enemy is blocking the next step on the current path return true.
	 ...
	 * 
	 * 
	 *         Worst Case Time Complexity: O(b^(d+1)) where b is the branching
	 *         factor and d is the depth of the goal node in the search tree.
	 * 
	 */
	private Stack<MapLocation> AstarSearch(MapLocation start, MapLocation goal, int xExtent, int yExtent,
		...
		// We initialize a Priority Queue to keep track of the nodes to be explored.
		// A HashSet is used to keep track of the nodes that have already been explored.
		// A HashMap is used to keep track of the best path costs for each node.

		PriorityQueue<MapLocation> space_to_explore = new PriorityQueue<MapLocation>(new costComparator());
		HashSet<MapLocation> explored = new HashSet<MapLocation>();
		HashMap<MapLocation, Float> best_path_costs = new HashMap<MapLocation, Float>();

		// add the start node to the queue and set its cost in the best path cost hash
		// map.

		space_to_explore.add(start);
		best_path_costs.put(start, start.g);

		do {
			MapLocation current = space_to_explore.poll();

			explored.add(current);

			// if current node has reached the goal node, then return the path from start to
			// current node.
			// O(n) time complexity.

			if (current.equals(goal)) {
				Stack<MapLocation> path = new Stack<MapLocation>();
				return tracePath(start, current.cameFrom, path);
			}

			// get all neighbors of the current node.

			ArrayList<MapLocation> neighbors = getAndCheckNeighbors(current.f, current, goal, xExtent, yExtent,
					enemyFootmanLoc, resourceLocations);

			// for each neighbor of the current node.

			for (MapLocation neighbor : neighbors) {
				if (!explored.contains(neighbor)) {
					if (best_path_costs.containsKey(neighbor)) {
						// if we have seen this node before, update its cost if a better path has been
						// found
						Float old_val = best_path_costs.get(neighbor);

						if (old_val > neighbor.f) {
							space_to_explore.remove(neighbor);
							space_to_explore.add(neighbor);
							best_path_costs.put(neighbor, best_path_costs.get(neighbor) + 1);
						}
					} else {
						// if this is the first time we have seen this node, add it to the queue and
						// update its cost
						space_to_explore.add(neighbor);
						best_path_costs.put(neighbor, neighbor.g);
					}
				}
			}
		} while (!space_to_explore.isEmpty());

		// if no path exists, return an empty stack

		return new Stack<MapLocation>();
	}

	/**
	 * A method to get a Stack of MapLocations representing a path from an initial
	 * node to some goal node with neither the initial or goal node appearing in the
	 * path. This method operates in linear runtime: O(P) where P is the size of the
	 * path.
	 * 
	...
	private Stack<MapLocation> tracePath(MapLocation start, MapLocation lastNode, Stack<MapLocation> path) {
		...
		// this works by tracing the path backwards from the lastNode until the parent =
		// start.
	...
	/**
	 * A method to get a list of MapPositions representing the neighbors of a node
	 * that are valid nodes to move to (valid nodes cannot be occupied by other
	 * objects such as trees,...).
	 * 
	 * @param parentGPlusEdgeCost The cost g(n) of the parent including the edge to
	 *                            the child we're going to generate
	 * @param pt                  The node to find the neighbors of.
	 * @param goal                The goal node, used to calculate h(n) from any
	 *                            neighbor.
	 * @param xExtent             The maximum x boundary of the environment
	 *                            (assuming 0 is min).
	 * @param yExtent             The maximum y boundary of the environment
	 *                            (assuming 0 is min).
	 * @param enemy               The location of the enemy in the environment (null
	 *                            if no enemy).
	 * @param obstacles           A set of obstacles in the environment represented
	 *                            as a set of positions that are already occupied.
	 * @return ArrayList of valid positions representing unoccupied neighbors of
	 *         node pt.
	 */
	private ArrayList<MapLocation> getAndCheckNeighbors(float parentGPlusEdgeCost, MapLocation pt, MapLocation goal,
			int xExtent, int yExtent, MapLocation enemy, Set<MapLocation> obstacles) {
		...
		if (enemy == null) {
			for (int i = pt.x - 1; i <= pt.x + 1; i++) { // run from pt.x-1 -> pt.x+1
				for (int j = pt.y - 1; j <= pt.y + 1; j++) { // run from pt.y-1 -> pt.y+1
					if (i != pt.x || j != pt.y) { // if we aren't at (pt.x, pt.y)
						newPt = new MapLocation(i, j, pt, parentGPlusEdgeCost,
								parentGPlusEdgeCost + this.heuristic(i, j, goal)); // neighbor point
			...
			for (int i = pt.x - 1; i <= pt.x + 1; i++) { // run from pt.x-1 -> pt.x+1
				for (int j = pt.y - 1; j <= pt.y + 1; j++) { // run from pt.y-1 -> pt.y+1
					if (i != pt.x || j != pt.y) { // if we arent at (pt.x, pt.y)
						newPt = new MapLocation(i, j, pt, parentGPlusEdgeCost,
								parentGPlusEdgeCost + this.heuristic(i, j, goal)); // neighbor point
	...
	/**
	 * A method to calculate the heuristic from any position to the goal node. This
	 * uses the Chebyshev distance D(x, y) = max(|x0 - x1|, |y0 -y1|).
	 * 
